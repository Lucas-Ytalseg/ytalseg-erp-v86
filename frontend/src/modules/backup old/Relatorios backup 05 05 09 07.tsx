...
function referenciaMesAno(mes: number, ano: number) {
  return `${nomeMes(mes)} / ${ano}`;
}

function semanaPorDataDinamica(dia: number, mes: number, ano: number) {
  const nomes = [
    "Domingo",
    "Segunda-feira",
    "Terça-feira",
...
export default function Relatorios() {
  ...
  useEffect(() => {
    try {
      ...
      if (data.mesAnalise) setMesAnalise(Number(data.mesAnalise));
      if (data.anoAnalise) setAnoAnalise(Number(data.anoAnalise));
  ...
  }, [empresaId, mesAnalise, anoAnalise, resultado, diasEditaveis, servicos]);
  ...
  function limparRelatorioNovo() {
    const ok = window.confirm("Limpar relatório atual e começar um novo?");
    if (!ok) return;
    ...
    localStorage.removeItem(RASCUNHO_KEY);
    setRascunhoStatus("Relatório limpo. Novo relatório iniciado.");
  }
  ...
  async function processarPdf() {
    ...
    try {
      ...
      setResultado(data);
      setDiasEditaveis(aplicarRegraEmpresaNosDias(montarDiasConferencia(data.calculo?.dias_detectados || [], mesAnalise, anoAnalise)));
    } catch (e: any) {
    ...
    setTimeout(async () => {
      try {
        if (window.ytalsegAPI?.salvarPDF) {
          const res = await window.ytalsegAPI.salvarPDF(`Relatorio_Cliente_${empresaAtual?.nome || "cliente"}.pdf`);
          if (res?.ok) setRascunhoStatus(`PDF cliente salvo: ${res.path || ""}`);
          else if (res?.erro && res.erro !== "cancelado") setRascunhoStatus(`Erro ao salvar PDF cliente: ${res.erro}`);
        } else {
          window.print();
        }
      } catch {
        setRascunhoStatus("Erro ao gerar PDF cliente.");
      } finally {
...
async function imprimirInterno() {
  ...
  setTimeout(async () => {
    try {
      if (window.ytalsegAPI?.salvarPDF) {
        const res = await window.ytalsegAPI.salvarPDF(`Relatorio_Interno_${empresaAtual?.nome || "interno"}.pdf`);
        if (res?.ok) setRascunhoStatus(`PDF interno salvo: ${res.path || ""}`);
        else if (res?.erro && res.erro !== "cancelado") setRascunhoStatus(`Erro ao salvar PDF interno: ${res.erro}`);
      } else {
        window.print();
      }
    } catch {
      setRascunhoStatus("Erro ao gerar PDF interno.");
    } finally {
      document.body.classList.remove("modo-interno");
    }
  }, 250);
        ...
        <div className="upload">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setMesAnalise(novoMes);
                setDiasEditaveis(montarDiasConferencia([], novoMes, anoAnalise));
              }}
            >
              {Array.from({ length: 12 }).map((_, i) => (
                <option key={i + 1} value={i + 1}>{nomeMes(i + 1)}</option>
              ))}
            </select>
          </div>
          ...
          <div>
            ...
            <input
              ...
              type="number"
              onChange={(e) => {
                const novoAno = Number(e.target.value || new Date().getFullYear());
                setAnoAnalise(novoAno);
                setDiasEditaveis(montarDiasConferencia([], mesAnalise, novoAno));
              }}
            />
          </div>
              <div>
...
<button className="btn-gray" onClick={imprimirInterno}>
  📊 PDF Interno
              ...
              <button className="btn-gray" onClick={gerarBackupManual}>
                Backup agora
              </button>
              <button className="btn-gray" onClick={limparRascunhoAutomatico}>
                Limpar rascunho
              </button>
              <button className="btn-gray" onClick={() => setZoomRelatorio(1.3)}>Zoom 180%</button>
              <button className="btn-gray" onClick={() => setZoomRelatorio(1.5)}>Zoom 200%</button>
              <button className="btn-gray" onClick={() => setZoomRelatorio(2)}>Zoom 230%</button>
