...
export default function Relatorios() {
  ...
  function limparTabelaToda() {
    const ok = window.confirm("Limpar toda a tabela de dias, serviços e resultado atual?");
    if (!ok) return;
  ...
  async function processarPdf() {
    ...
    try {
      ...
      setResultado(data);
      const diasApi = (data.calculo?.dias_detectados || []).map((d: any) => {
        const partes = String(d.data || "").split("/");
        if (partes.length === 3) {
          const dia = partes[0].padStart(2, "0");
          const semanaCorrigida = semanaPorDataDinamica(Number(dia), mesAnalise, anoAnalise);
          return {
            ...d,
            data: `${dia}/${String(mesAnalise).padStart(2, "0")}/${anoAnalise}`,
            semana: semanaCorrigida,
            tipo_dia: tipoDiaPorSemana(semanaCorrigida),
          };
        }
        return d;
      });

      setDiasEditaveis(aplicarRegraEmpresaNosDias(montarDiasConferencia(diasApi, mesAnalise, anoAnalise)));
    } catch (e: any) {
...
<button className="btn-gray" onClick={imprimirInterno}>
  📊 PDF Interno
              ...
              <button className="btn-red" onClick={limparTabelaToda}>
                Limpar tabela
              </button>
