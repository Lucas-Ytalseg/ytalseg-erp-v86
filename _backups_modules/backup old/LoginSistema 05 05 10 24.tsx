...
export default function LoginSistema({
  ...
  function entrar() {
    ...
    if (!usuario) {
      setMsg("Usuário/senha inválidos ou usuário inativo.");
      return;
    }
  ...
  function resetarUsuariosPadrao() {
    const ok = window.confirm("Restaurar usuários padrão? Isso não apaga dados do sistema.");
    if (!ok) return;
  ...
  return (
    <div className="login-page">
      <style>{`
        ...
        .login-logo { color: #00B050; font-size: 42px; font-weight: 1000; letter-spacing: -1px; line-height: .9; margin-bottom: 8px; }
        .login-sub { color: #6b7280; font-weight: 800; margin-bottom: 22px; }
        .login-label { display: block; font-size: 12px; font-weight: 1000; color: #006b34; margin: 12px 0 6px; text-transform: uppercase; }
        .login-input { width: 100%; border: 1px solid #d1d5db; border-radius: 12px; padding: 12px 14px; font-size: 15px; font-weight: 800; }
        .login-btn { width: 100%; border: 0; border-radius: 12px; padding: 12px 14px; font-size: 15px; font-weight: 1000; cursor: pointer; background: #00B050; color: white; margin-top: 16px; }
        .login-btn-gray { width: 100%; border: 0; border-radius: 12px; padding: 10px 12px; font-size: 13px; font-weight: 900; cursor: pointer; background: #e5e7eb; color: #111; margin-top: 10px; }
      ...
      <div className="login-card">
        ...
        <button className="login-btn-gray" onClick={() => setMostrarAjuda(!mostrarAjuda)}>{mostrarAjuda ? "Ocultar ajuda" : "Mostrar login padrão"}</button>
        ...
        {mostrarAjuda && (
          <div className="login-help">
            ...
            financeiro / 123<br />
            consulta / 123
            <button className="login-btn-gray" onClick={resetarUsuariosPadrao}>Restaurar usuários padrão</button>
          </div>
        )}
      </div>
    </div>
